...
#define SSD1306_I2C_ADDR 0x3C // Adresse I2C typique pour SSD1306
...
static uint8_t framebuffer[128][4] = {0}; // 128 colonnes x 4 pages (pour 128x32)
...
static void ssd1306_init(const struct device *i2c_dev) {
    ...
    ssd1306_write_cmd(i2c_dev, 0x1F); // 1/32 duty (0x1F pour 32 lignes)
    ...
    ssd1306_write_cmd(i2c_dev, 0x02); // 0x02 pour 128x32 (voir datasheet)
    ssd1306_write_cmd(i2c_dev, 0x81); // Set contrast control
    ssd1306_write_cmd(i2c_dev, 0x7F);
...
void clear_display(const struct device *i2c_dev) {
	    ...
	    for (uint8_t page = 0; page < 4; page++) { // 4 pages pour 32 lignes
        ssd1306_write_cmd(i2c_dev, 0x21); // Set column address
...
y+=8;
x=0;
const char *line3 = "Vitesse: 50 km/h";
