...
int main(void)
{
	...
	while(1){
		...
		int measure = batt_measure();
		snprintf(message, sizeof(message), "BATTERY: %d mV", measure);
		write_line(i2c_dev, message, 0, 0, 1);
	...
	send_data_len = snprintf(message2, 2000,
									"POST %s HTTP/1.1\r\n"
                                    "Host: %s\r\n\r\n"
									"%d",
									"/batt", HTTP_HOST,
    ...
    do {
        num_bytes =
        blocking_send(client_fd, message2, send_data_len, 0);
	...
	k_sleep(K_SECONDS(60)); // Attendre 60 secondes avant de renvoyer la mesure de la batterie
